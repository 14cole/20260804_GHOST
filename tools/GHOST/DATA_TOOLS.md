...
Conversion automatically discovers a sibling `GRIM_Backend` or `GRIM`
checkout. If it lives elsewhere, point to that project's folder:

```bash
export GRIM_BACKEND_PATH=/path/to/GRIM_Backend
export CEM_SOLVER_BACKEND_PATH=/path/to/solver-project/ghost_backend
```
...
The `subtract` command reads the supplied OPN and FRD folders, joins compatible
frequency files internally, and preserves the solver's VV/HH fields. From the
GHOST folder, run `python ghost_backend/data_tools/run_cli.py subtract OPN FRD Deltas`.
The `concat-pols` and `concat-freqs` commands also support general dataset work.
...
- CSV/TXT output uses the same `grim.flat-rcs.v1` schema as GRIM Plotting.
  ...
  See `GRIM_Backend/docs/README.md` for the complete column contract.
...
same placement-ready delta convention consumed by `ghost_backend/assembly/place_features.py`.
