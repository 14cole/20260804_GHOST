...
Promoted from experiments/round2 and round5. No runtime dependency on the
playground; all changes are selected explicitly by cpu_execution state.
"""
import hashlib
import time
import numpy as np
...
from cpu_execution import current_state, TABLE_BYTES
...
def farfield(mesh,density,k_air,observation_angles_deg,potential,order=8,
    ...
    # Roughly 20 MB per main temporary; no (angle,element,quadrature) tensors.
...
@timed_stage("excitation")
def incident_loads(mesh,k,angles,bu=None,bdn=None):
    dirs=directions(angles);ids,centers,edges,lengths,normals=geometry_arrays(mesh)
    if bu is None:bu=np.zeros((len(mesh.nodes),len(dirs)),dtype=np.complex128)
    if bdn is None:bdn=np.zeros_like(bu)
    ...
    for start in range(0,len(ids),block):
        ...
        dn=1j*float(k)*dot2(normals[part]);i0*=dn;i1*=dn
...
class KernelTable:
    def __init__(self,k,upper,degree=16,tolerance=2e-13):
        ...
        while bounds[-1]<upper:
            bounds.append(min(upper,bounds[-1]*1.25,bounds[-1]+1./abs(k)))
        ...
        # Convert T_j(2*t-1) once; PPoly uses powers of r-left_endpoint.
        ...
        self.evidence=dict(k_real=k.real,k_imag=k.imag,degree=degree,intervals=len(bounds)-1,
            bytes=sum(p.c.nbytes for p in self.polys)+self.bounds.nbytes,check_points=rr.size,
            max_check_relative=error,max_tail_relative=tail,tolerance=tolerance,build_seconds=time.perf_counter()-start)



def select_far_kernels(mesh, k, green, hankel):
    ...
    if key in state.tables:
        ...
        # Bound rejected-key bookkeeping as well as coefficient storage.
    ...
    def evaluator(channel, original):
        def evaluate(k0, real_k, dist, kr, scratch, out):
            ...
            bad = ~np.isfinite(out)
            if np.any(bad):
                fallback = np.empty_like(out)
                original(k0, real_k, dist, kr, scratch, fallback)
