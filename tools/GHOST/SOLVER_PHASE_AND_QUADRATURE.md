# Solver phase compatibility and quadrature

The solver implements the following phase, storage and quadrature behavior.
...
Regression coverage in `ghost_backend/tests/test_solver_audit_fixes.py` checks
absolute cylindrical complex amplitudes, certified phase, lossless energy,
incidence-count planning, frequency-local caches, mesh counts, independently
integrated angular kernels, compact near storage, sparse constraint algebra,
and an analytical coated-sphere case. Coverage applies to those tested cases.
