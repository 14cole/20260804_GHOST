...
"<POL>_<FREQ:.3f>GHz_<geometry_stem>.grim". Every resumable output carries an
exact byte/source/runtime/input attestation; an unverified existing file is
rejected. Each unit uses the solver's threaded mode engine, so the pool default
is few processes x several threads.
"""
