...
from bor_solver import (BOR_CONDITION_EST_MAX,
                        ...
                        _causal_medium, _MultiRegionBor, _solve_multiregion)
...
def bor_az_el_grid(res_vv: 'Dict[str, Any]', res_hh: 'Dict[str, Any]',
    ...
    if not isinstance(res_vv, dict) or not isinstance(res_hh, dict):
        raise ValueError("VV and HH inputs must be BoR result dictionaries.")
    if "polarization" not in res_vv or "polarization" not in res_hh:
        raise ValueError("VV and HH results must declare their polarization.")
    ...
    def grid_axis(values, name):
        try:
            arr = np.asarray(values, dtype=float)
        except (TypeError, ValueError) as exc:
            raise ValueError(f"{name} must contain numeric degree values.") from exc
        if arr.ndim == 0:
            arr = arr.reshape(1)
        if arr.ndim != 1 or arr.size == 0:
            raise ValueError(f"{name} must be a non-empty one-dimensional axis.")
        if not np.all(np.isfinite(arr)):
            raise ValueError(f"{name} must contain only finite degree values.")
        if arr.size > 1 and np.any(np.diff(arr) <= 0.0):
            raise ValueError(f"{name} must be unique and strictly increasing.")
        return arr

    az = grid_axis(azimuths_deg, "azimuths_deg")
    el = grid_axis(elevations_deg, "elevations_deg")
    if np.any((el < -90.0) | (el > 90.0)):
        raise ValueError("elevations_deg must lie in [-90, 90] degrees.")
    if np.unique(np.mod(az, 360.0)).size != az.size:
        raise ValueError(
            "azimuths_deg contains duplicate directions modulo 360 degrees."
        )
    try:
        axis_az_raw = np.asarray(axis_az_deg)
        axis_el_raw = np.asarray(axis_el_deg)
        if axis_az_raw.ndim != 0 or axis_el_raw.ndim != 0:
            raise ValueError
        axis_az = float(axis_az_raw)
        axis_el = float(axis_el_raw)
    except (TypeError, ValueError) as exc:
        raise ValueError("axis_az_deg and axis_el_deg must be numeric scalars.") from exc
    if not math.isfinite(axis_az) or not math.isfinite(axis_el):
        raise ValueError("axis_az_deg and axis_el_deg must be finite.")
    if axis_el < -90.0 or axis_el > 90.0:
        raise ValueError("axis_el_deg must lie in [-90, 90] degrees.")

    def by_freq(res, label):
        samples = res.get("samples")
        if not isinstance(samples, (list, tuple)) or not samples:
            raise ValueError(f"{label} result contains no samples.")
        out: 'Dict[float, Tuple[np.ndarray, np.ndarray]]' = {}
        entries: 'Dict[float, List[Tuple[float, complex]]]' = {}
        for index, sample in enumerate(samples):
            if not isinstance(sample, dict):
                raise ValueError(f"{label} sample {index} is not a dictionary.")
            try:
                freq = float(sample["frequency_ghz"])
                theta = float(sample["theta_inc_deg"])
                amplitude = complex(
                    float(sample["rcs_amp_real"]),
                    float(sample["rcs_amp_imag"]),
                )
            except (KeyError, TypeError, ValueError) as exc:
                raise ValueError(
                    f"{label} sample {index} is missing finite numeric "
                    "frequency, aspect, or complex-amplitude data."
                ) from exc
            if not math.isfinite(freq) or freq <= 0.0:
                raise ValueError(
                    f"{label} sample {index} has invalid frequency {freq!r}."
                )
            if not math.isfinite(theta) or theta < 0.0 or theta > 180.0:
                raise ValueError(
                    f"{label} sample {index} has invalid BoR aspect {theta!r}; "
                    "expected a finite value in [0, 180] degrees."
                )
            if not (
                math.isfinite(amplitude.real) and math.isfinite(amplitude.imag)
            ):
                raise ValueError(
                    f"{label} sample {index} has a non-finite complex amplitude."
                )
            entries.setdefault(freq, []).append((theta, amplitude))

        for freq, values in entries.items():
            theta = np.asarray([value[0] for value in values], dtype=float)
            amplitude = np.asarray([value[1] for value in values], dtype=complex)
            if theta.size > 1 and np.any(np.diff(theta) <= 0.0):
                raise ValueError(
                    f"{label} aspect grid at {freq:g} GHz must be unique and "
                    "strictly increasing in sample order."
                )
            out[freq] = (theta, amplitude)
        return out

    fv, fh = by_freq(res_vv, "VV"), by_freq(res_hh, "HH")
    freqs = sorted(fv)
    if sorted(fh) != freqs:
    ...
    for freq in freqs:
        th_v = fv[freq][0]
        th_h = fh[freq][0]
        if not np.array_equal(th_v, th_h):
            raise ValueError(
                f"VV and HH aspect grids at {freq:g} GHz must match exactly."
            )

    def unit(v):
        norm = float(np.linalg.norm(v))
        if not math.isfinite(norm) or norm <= 0.0:
            raise ValueError("Cannot construct a finite polarization basis.")
        return v / norm
    ...
    ax = direction(axis_az, axis_el)
    basis_tol = 1.0e-12

    theta_map = np.zeros((len(az), len(el)))
    m_maps = np.zeros((len(az), len(el), 2, 2))
    axis_aligned = np.zeros((len(az), len(el)), dtype=bool)
    for i, a_deg in enumerate(az):
        az_rad = math.radians(float(a_deg))
        # Standard spherical e_phi.  Unlike cross(z_hat, d), this retains the
        # requested azimuthal polarization reference at vertical looks.
        h_r = np.array([-math.sin(az_rad), math.cos(az_rad), 0.0])
        for j, e_deg in enumerate(el):
            d = direction(float(a_deg), float(e_deg))
            v_r = unit(np.cross(h_r, d))
            ct = float(np.clip(np.dot(d, ax), -1.0, 1.0))
            theta_map[i, j] = math.degrees(math.acos(ct))
            c = np.cross(ax, d)
            if np.linalg.norm(c) <= basis_tol:
                # The target meridian is undefined on the rotation axis.
                # Choose the deterministic radar basis itself; the response
                # is basis-independent only when axial VV == HH, which is
                # checked (and tiny numerical differences isotropized) below.
                axis_aligned[i, j] = True
                m_maps[i, j] = np.eye(2)
                continue
            h_t = unit(c)
    ...
    thq = theta_map.ravel()
    query_min = float(np.min(thq))
    query_max = float(np.max(thq))
    axial_flat = axis_aligned.ravel()
    for kf, f in enumerate(freqs):
        th_v, av = fv[f]
        th_h, ah = fh[f]
        if query_min < float(th_v[0]) or query_max > float(th_v[-1]):
            raise ValueError(
                f"Aspect grid at {f:g} GHz covers "
                f"[{float(th_v[0]):.12g}, {float(th_v[-1]):.12g}] degrees, "
                f"but the requested az/el grid requires "
                f"[{query_min:.12g}, {query_max:.12g}] degrees; endpoint "
        ...
        if np.any(axial_flat):
            delta = np.abs(Fv[axial_flat] - Fh[axial_flat])
            scale = np.maximum(
                np.maximum(np.abs(Fv[axial_flat]), np.abs(Fh[axial_flat])),
                1.0e-15,
            )
            if np.any(delta > 1.0e-8 * scale):
                worst = float(np.max(delta / scale))
                raise ValueError(
                    f"VV and HH axial amplitudes at {f:g} GHz violate BoR "
                    f"isotropy (worst relative mismatch {worst:.6g})."
                )
            axial_mean = 0.5 * (Fv[axial_flat] + Fh[axial_flat])
            Fv[axial_flat] = axial_mean
            Fh[axial_flat] = axial_mean
