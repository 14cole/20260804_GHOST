...
if not exist "GRIM_Backend\run_gui.py" (
    echo ERROR: GRIM_Backend\run_gui.py was not found.
...
if not errorlevel 1 (
    ...
    py.exe -3 -c "import sys; sys.path.insert(0, r'%~dp0.'); import GRIM_Backend.ui.app" >>"%GRIM_LAUNCH_LOG%" 2>&1
...
"%GRIM_PYTHON%" -c "import sys; sys.path.insert(0, r'%~dp0.'); import GRIM_Backend.ui.app" >>"%GRIM_LAUNCH_LOG%" 2>&1
exit /b %ERRORLEVEL%

:launch_python
for %%I in ("%GRIM_PYTHON%") do set "GRIM_PYTHONW=%%~dpIpythonw.exe"
if exist "%GRIM_PYTHONW%" (
    start "" "%GRIM_PYTHONW%" "%~dp0GRIM_Backend\run_gui.py"
) else (
    start "" "%GRIM_PYTHON%" "%~dp0GRIM_Backend\run_gui.py"
)
exit /b 0
...
if errorlevel 1 (
    start "" py.exe -3 "%~dp0GRIM_Backend\run_gui.py"
) else (
    start "" pyw.exe -3 "%~dp0GRIM_Backend\run_gui.py"
)
exit /b 0
