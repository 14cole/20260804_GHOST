...
from ppt_workspace import PptWorkspace, DatasetCatalogEntry
from ppt_workflow import write_report_recipe, read_report_recipe, inspect_template, current_plot_report_setup
from assembly_workflow import suggest_response_mapping
from feature_assembly_panel import FeatureAssemblyPanel, read_feature_assembly_recipe
...
class WorkflowUpdatesTests(unittest.TestCase):
    ...
    def test_physical_dimensions_and_preflight_material_errors(self):
        ...
        if not geo.is_file():
            geo=next((Path(__file__).resolve().parents[1]/'tools/GHOST/geometry_tests/material_examples').glob('*pec*.geo'))
    ...
    def test_ppt_template_probe_closes_owned_copy_on_layout_error(self):
        from ppt_report import PowerPointComBridge
