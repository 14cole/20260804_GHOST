...
class StreamedOperator:
    def __init__(self,oracle,coordinates,tile=128,tolerance=1e-14,budget=512*1024**2,checkpoint=None,compression='qr'):
        ...
        for i,rows in enumerate(self.groups):
            for j,cols in enumerate(self.groups):
                ...
                payload=(raw,None)
                if i!=j:
                    payload,raw,tail,accepted=tile_payload(raw,tail,tolerance,compression)
                    compressed+=int(accepted)
        ...
        self.evidence=dict(unknowns=self.n,tiles=len(self.tiles),compressed_tiles=compressed,
            ...
            row_error_bound=float(self.row_error.max()),storage_budget=budget,compression=compression)
