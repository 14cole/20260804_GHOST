...
Accepts geometry or stored-tile coefficient oracles. Retained numeric storage
is capped separately from bounded construction workspaces and process RSS.
No fallback builds a dense system larger than 512 unknowns.
"""
from common import *
...
class CompressedSystem:
    def __init__(self,oracle,coordinates,tolerance=1e-12,leaf=128,budget=512*1024**2,checkpoint=None):
        ...
        # Avoid sorting/copying the same column IDs for every ACA row query.
        # All tree/block indices derive from the validated permutation above.
