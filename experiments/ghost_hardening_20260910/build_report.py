"""Render the follow-up report from validated, saved measurements."""
from pathlib import Path
import json
import numpy as np
here=Path(__file__).resolve().parent
root=here.parents[1]
r=json.loads((here/'validated-results.json').read_text())
native=json.loads((here/'native-matrix-results.json').read_text())
def link(label,path):return '[{}]({})'.format(label,path.as_posix())
current_hash=json.loads((here/'final-source-hashes.json').read_text())['sweep_compression.py']
def metric(label,key):return [v[key] for name,v in r['production'].items() if label in name and
    ('updated' not in label or v['sweep_sha256']==current_hash)]
before=metric('baseline-dense','seconds');after=metric('updated-dense','seconds')
before_ram=metric('baseline-dense','peak_mib');after_ram=metric('updated-dense','peak_mib')
h=r['production']['airfoil-f2.0-updated-hierarchical-r1.json']
lines=['# GHOST 2D follow-up: hardening and measured bottlenecks',
...
'| Previous production, dense | {:.2f} s median | {:.0f}–{:.0f} MiB |'.format(np.median(before),min(before_ram),max(before_ram)),
'| Final production, dense | {:.2f} s | {:.0f} MiB |'.format(np.median(after),np.median(after_ram)),
'| Hierarchical, before final norm change | {:.2f} s | {:.0f} MiB |'.format(h['seconds'],h['peak_mib']),
...
'|---|---:|---:|---:|---:|---:|---:|']
for pol in ('TE','TM'):
    s=r['streamed']['streamed-f2.0-{}-tile512-cut32.0-qr.json'.format(pol)];b=s['baseline']
    core=lambda v:sum(v[k] for k in ('assembly_seconds','inverse_seconds','solve_and_bound_seconds'))
    lines.append('| {} | {:.2f} s | {:.2f} s | {:.2f} s | {:.2f} s | {:.0f} MiB | {:.0f} MiB |'.format(pol,b['assembly_seconds'],s['assembly_seconds'],core(b),core(s),s['construction_sampled_rss_bytes']/2**20,s['assembly_inverse_rhs_and_check_sampled_rss_bytes']/2**20))
maximum=lambda key:max(s[key] for s in r['streamed'].values())
lines.extend([
'All four 1/2 GHz TE/TM cases passed all 361 original RHS checks and independent dense-field comparisons. Largest independent backward error: {:.3g}; largest complex-field difference relative to the channel peak: {:.3g}; largest coefficient-error-inclusive solve bound: {:.3g}. Construction inputs released immediately. Every final coefficient was requested once from geometry (D² entries); subsequent inverse queries read compressed tiles.'.format(maximum('independent_backward'),maximum('field_error'),maximum('original_residual_bound')),
...
'Evidence: '+link('validated measurements',here/'validated-results.json')+', '+link('native material results',here/'native-matrix-results.json')+', '+link('Backend changes from this pass',here/'hardening.diff')+', '+link('source hashes',here/'final-source-hashes.json')+'.',
