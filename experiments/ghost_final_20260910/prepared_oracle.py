...
class PreparedOracle:
    ...
    def get_with_error(self,rows,cols,assemble=True):
        ...
        assemble_groups(self.mesh,self.geometry,pending)
        return matrix,error


def assemble_groups(mesh,geometry,groups):
    for k,outputs,masks,coefficients in groups:
        rcs._assemble_linear_operator_matrices_multi(mesh,k,True,masks,
            ...
            operator_outputs=outputs,prepared_geometry=geometry)


class PairedOracle:
    """TE/TM destinations share geometry/kernel traversal, preserving each law."""
    def __init__(self,mesh,te_infos,tm_infos,cut=None):
        self.oracles=[PreparedOracle(mesh,te_infos,'TE',cut),PreparedOracle(mesh,tm_infos,'TM',cut)]
        a,b=self.oracles
    ...
    def get_with_error(self,rows,cols):
        ...
        assemble_groups(self.mesh,self.oracles[0].geometry,[(k,)+tuple(v) for k,v in grouped.items()])
